...
export default function TextHitLayer({
  ...
  // Single-span selection is the safe default for forms and bilingual rows.
  // Paragraph mode remains available via the badge / double-click.
  const [granularity, setGranularity] = useState<Granularity>("span");

  const handleLineClick = useCallback(
    (e: React.MouseEvent<HTMLDivElement>, para: ExtractedParagraph, line: ExtractedLine) => {
      if (e.altKey || granularity === "span") {
        // Alt-click or span-mode: find nearest span within this line only
        // offsetX/Y are relative to this line's hit rectangle, while span
        // bboxes are page-relative. Convert both to the same coordinate space.
        const canvasX = e.currentTarget.offsetLeft + e.nativeEvent.offsetX;
        const canvasY = e.currentTarget.offsetTop + e.nativeEvent.offsetY;
  ...
  const editableParagraphs = paragraphs.filter((p) =>
    p.lines.some((l) => l.spans.length > 0)
  );
  ...
  return (
    ...
    >
      ...
      {editableParagraphs.map((para) => {
        const editableLines = para.lines.filter((l) => l.spans.length > 0);
