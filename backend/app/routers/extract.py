...
def _sample_background_color(
    ...
    for x, y in points:
        px = min(max(int(round(x)), 0), max(int(page_w) - 1, 0))
        py = min(max(int(round(y)), 0), max(int(page_h) - 1, 0))
        color = pix.pixel(px, py)
        if len(color) >= 3:
            samples.append((color[0], color[1], color[2]))
    if not samples:
        return "#ffffff"
    rgb = tuple(round(statistics.median(channel)) for channel in zip(*samples))
...
def _dominant_style(line_spans: list[dict[str, Any]]) -> tuple[bool, bool]:
    ...
    for s in line_spans:
        ...
        if tl > best_len:
            ...
            best_bold = bool(flags & (1 << 4))
            best_italic = bool(flags & (1 << 1))
    ...
    for block in text_blocks:
        ...
        for line_idx, line in enumerate(block.get("lines", [])):
            ...
            for span_idx, span in enumerate(line.get("spans", [])):
                ...
                origin = list(span.get("origin", [sbbox[0], sbbox[3]]))
                asc_raw = float(span.get("ascender", 0.83))
                desc_raw = float(span.get("descender", -0.21))
                ...
                raw_spans.append({
                    ...
                    "fontName":    font_name,
                    "dir":         [round(d, 4) for d in dir_vec],
                    "isLtr":       abs(dir_vec[0] - 1.0) < 0.05 and abs(dir_vec[1]) < 0.05,
        ...
        for run in para_runs:
            if not run:
                continue
            all_span_ids = [s["spanId"] for ln in run for s in ln["spans"]]
